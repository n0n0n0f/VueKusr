<template>
  <div class="book-detail">
    <div v-if="book" class="book-info-container">
      ...
      <ul class="reviews-list">
        <li v-for="review in reviews" :key="review.id" class="review-item">
          ...
          <button
            ...
            @click="deleteReviewAction(review.id)"
            class="delete-button"
          >
            Удалить
          </button>
      ...
      <h2 class="section-title" v-if="!isLibrarian && !isSystemAdmin">
        Добавить отзыв
      </h2>
      <div
        v-if="isAuthenticated && !isLibrarian && !isSystemAdmin"
      ...
      >
        ...
        <p v-if="book.category !== 1" class="reservation-message">
          Эту книгу нельзя забронировать.
        </p>
      </div>
      <div v-else-if="!isAuthenticated">
...
export default {
  ...
  methods: {
    ...
    async loadReviews() {
      if (this.book) await this.fetchReviews(this.book.id);
    },
    ...
    async reserveBook() {
      if (this.book && this.book.category === 1) {
        try {
          ...
          const alreadyReserved = this.reservedBooks.some(
            (reservation) =>
              reservation.book_id === bookId && reservation.user_id === userId
          );
          if (alreadyReserved)
            ...
            this.reservationMessage =
              "Запрос на бронирование отправлен библиотекарю.";
          }
        } catch (error) {
          this.reservationMessage =
            "Ошибка при отправке запроса на бронирование.";
  ...
  async created() {
    if (!this.book) await this.fetchBooks();
    await this.loadReviews();
  },
  watch: {
    book(newBook) {
      if (newBook) this.loadReviews();
    },
  },
};
</script>
